...
msmrtc_probe(struct platform_device *pdev)
{
	...
	/* Set the default system time to 2009-12-01 00:00:00 UTC.
	 */
	msmrtc_timeremote_read_time(&pdev->dev, &tm);
	if (tm.tm_year < 90) {
		tm.tm_year = 109; /* RTC layer expects years to start at 1900 */
		tm.tm_mon  = 11;  /* RTC layer expects mons to be 0 based */
		tm.tm_mday = 1;
		tm.tm_hour = 0;
		tm.tm_min  = 0;
		tm.tm_sec  = 0;
		tm.tm_wday = 1;   /* 2009-12-01 is Tues. */
