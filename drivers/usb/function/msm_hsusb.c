...
#define CHARGE_AC	900
#define CHARGE_USB_DEFAULT	100
#define CHARGE_USB_CONFIGURED	500
...
#if !defined(CONFIG_USB_GOOGLE_VID_HACK)

#if defined(CONFIG_MACH_ACER_A1)
static int pid = 0x3203; // ADB disabled
#else
...
static int usb_is_online(struct usb_info *ui)
{
	...
	return -1;
#else
	unsigned int	xUSB_USBCMD;
	unsigned int	xUSB_USBSTS;
	unsigned int	xUSB_PORTSC;

	xUSB_USBCMD=readl(USB_USBCMD);
	xUSB_USBSTS=readl(USB_USBSTS);
	xUSB_PORTSC=readl(USB_PORTSC);
	/* //A1 not support Host mode
	if (((xUSB_PORTSC) & PORTSC_SUSP) == PORTSC_SUSP) {
		pr_info("%s::usb is offline::host mode:1 = Port in suspend state\n",__func__);
		return 0;
	}
	*/
	if ((((xUSB_PORTSC) & PORTSC_CCS) == 0) ||
	    (((xUSB_USBCMD) & USBCMD_RS) == 0)) {
		pr_debug("%s::usbcmd:(%08x) usbsts:(%08x) portsc:(%08x) %s\n",
			__func__,
			(xUSB_USBCMD),
			(xUSB_USBSTS),
			(xUSB_PORTSC),
			(((xUSB_PORTSC) & PORTSC_CCS) == 0)?"Not attached\n":
			(((xUSB_USBCMD) & USBCMD_RS) == 0)?"Stop":""
		);
		return 0;
	}
	pr_debug("%s::usb is online\n",__func__);
	return -1;
		...
		case USB_STATE_ONLINE:
			...
			if ((flags & USB_FLAG_RESUME) ||
				...
				if (ui->online) {
					...
					ui->usb_state = USB_STATE_DEFAULT;
					msm_chg_usb_i_is_available(CHARGE_USB_DEFAULT);
				}
				break;
			}
			goto reset;
...
static void usb_chg_set_type(struct usb_info *ui)
{
	...
	case CHG_WALL:
		...
		msm_chg_usb_i_is_available(CHARGE_AC);
		break;
	case CHG_HOST_PC:
...
#if defined(CONFIG_MACH_ACER_A1)
static int is_exist_serial_number(char* str)
{
	int i =0 ;
	for(;i<10;i++)
		if(str[i] != 0)
...
#if defined(CONFIG_MACH_ACER_A1)
	if( is_exist_serial_number(ui->pdata->serial_number))
#else
	if (ui->pdata->serial_number)
#endif
		desc_device.iSerialNumber =
			usb_msm_get_next_strdesc_id(ui->pdata->serial_number);
	pr_debug("usb serial_number = %s, size=%d, iSerialNumber = %d\n", ui->pdata->serial_number, sizeof(ui->pdata->serial_number), desc_device.iSerialNumber);
...
static int __init usb_probe(struct platform_device *pdev)
{
	...
	ui->selfpowered = 0;
	ui->remote_wakeup = 0;
	ui->maxpower = CHARGE_USB_CONFIGURED/2;
	ui->chg_type = CHG_UNDEFINED;
...
static void hsusb_early_suspend(struct early_suspend *h)
{
	...
	pr_info("USB early_suspend ui->usb_state=%d, wlock_flags=0x%x, is_online=%d\n", ui->usb_state, ui->wlock.flags, usb_is_online(ui));
	if(!usb_is_online(ui) && !!(ui->wlock.flags & WAKE_LOCK_ACTIVE) && !(ui->wlock.flags & WAKE_LOCK_AUTO_EXPIRE)){
		/* USB offline, wake-lock active but no auto-expire */
		pr_info("USB Wakelock Workaround: Call release wake-lock (flags=0x%x)\n", ui->wlock.flags);
		usb_lpm_enter(ui);
		msm_hsusb_suspend_locks_acquire(ui, 0); /* Release the wake-lock (half second timeout) */
	}
}
#endif
#endif
