...
static struct flash_identification supported_flash[] =
{
	...
	{0x5501bcec, 0xFF00FFFF, (512<<20), 1, 2048, (2048<<6), 64, }, /*Sams*/ /* rbtchc: for SAMSUNG_KBY00N00HM */
...
#if 0 /* test mark bad block */
	{
		loff_t xx;
		unsigned i;
		static int enter[5]={0,0,0,0,0};
		for (i=0; i<sizeof(enter)/sizeof(int); i++) {
			xx=	(mtd->erasesize*(1529+i))+	/* block */
				(mtd->writesize*0);	/* page */
			if (to==xx) {
				pr_err("@@ test %s: write bad address, 0x%llx\n", __func__, to);
				if (enter[i]==0) {
					enter[i]++;
					pr_err("@@ test [fail] %s: write bad address, 0x%llx\n", __func__, to);
					return -EIO;
				}
			}
		}
...
static int
msm_nand_block_isbad2(struct mtd_info *mtd, loff_t ofs)
{
	struct msm_nand_chip *chip = mtd->priv;
	int ret;
	struct {
...
msm_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (msm_nand_block_isbad2(mtd, ofs + mtd->writesize)==1) {
		pr_err("%s: bad block address 0x%x, block %d fail\n",
			__func__, (uint32_t)ofs,
			(uint32_t)ofs/mtd->erasesize);
		return 1; /* bad block */
	}
	r=msm_nand_block_isbad2(mtd, ofs);
	if (r==1) {
		pr_err("%s: bad block address 0x%x, block %d fail\n",
			__func__, (uint32_t)ofs,
			(uint32_t)ofs/mtd->erasesize);
...
msm_nand_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	pr_err("%s: mark bad/fail address, 0x%llx\n", __func__, ofs);
	/* Check for invalid offset */
	if (ofs > mtd->size)
		return -EINVAL;
	if (ofs & (mtd->erasesize - 1)) {
	...
	*/
	...
	if ( ret!=0 ) {
		pr_err("fail: %s: mark #1 bad address, 0x%llx\n",
		       __func__, ofs);
	}
	ofs2 = ofs + mtd->writesize;
	ret2 =  msm_nand_write_oob(mtd, ofs2, &ops);
	if ( ret2!=0 ) {
		pr_err("fail: %s: mark #2 bad address, 0x%llx (0x%llx)\n",
		       __func__, ofs2, ofs);
	}
	if ((ret==0) || (ret2==0)) {
		return 0;
	...
	pr_err("eoor: %s: mark bad address, 0x%llx\n", __func__, ofs);
	return ret;
}
